...
const formatContent = (text) => {
  ...
  safeText = safeText.replace(/\[FILE:([^|]+)\|([^\]]+)\]/g, '<a href="$2" target="_blank" style="color: #59B3D9; text-decoration: underline; font-weight: bold;">?ìé $1</a>');
...
const submitAdminReply = async (id) => {
  ...
  try {
    ...
    if (error) {
      alert("?∞Ïù¥?∞Î≤†?¥Ïä§ ?§Î•ò: " + error.message)
      return
    }
    
    alert('?µÎ????Ä?•Îêò?àÏäµ?àÎã§.')
    ...
    alert("?§Î•ò Î∞úÏÉù: " + err.message)
  }
}

const copyEmail = () => {
  navigator.clipboard.writeText('contact@c-braindesign.com')
  alert(state.currentLang === 'ko' ? '?¥Î©î??Ï£ºÏÜåÍ∞Ä Î≥µÏÇ¨?òÏóà?µÎãà??' : 'Email address copied!')
}
  ...
  <div class="app-container">
    ...
    <div class="top-utility-bar">
      <div class="utility-content">
        ...
        <template v-if="state.currentUser">
           <a href="#" @click.prevent="handleLogout">Î°úÍ∑∏?ÑÏõÉ</a>
        </template>
        <template v-else>
      ...
      <div class="nav-content">
        ...
        <div class="nav-actions">
          ...
          <a href="#" class="icon-btn hide-mobile" @click.prevent="goToCart">
            ?õí <span v-if="state.cart.length > 0" class="cart-badge">{{ state.cart.length }}</span>
          </a>
          <a href="#" class="icon-btn hide-mobile my-page-icon-wrapper" @click.prevent="goToMyPage">
            <span class="icon-emoji">?ë§</span>
    ...
    <div class="mobile-slide-menu" :class="{ 'is-open': state.isMobileMenuOpen }">
      ...
      <nav class="mobile-nav-links">
        ...
        <a href="#" @click.prevent="goToJobs()">{{ t.nav.jobs }}</a>
        <a href="#" @click.prevent="goToCart()">?•Î∞îÍµ¨Îãà ({{ state.cart.length }})</a>
        <a href="#" @click.prevent="goToMyPage()">ÎßàÏù¥?òÏù¥ÏßÄ</a>
      ...
      <div class="tech-modal glass-panel">
        ...
        <div class="modal-body">
          <div class="detail-row">
            <span class="label">?†Ïßú/?ºÏãú:</span>
          ...
          <div v-if="state.selectedItemDetail?.order_status" class="order-detail-container">
            <div class="detail-section">
              <h4 class="detail-sec-title">?ì¶ ?ÅÌíà ?ïÎ≥¥</h4>
              <div class="detail-data-box">
                <div class="data-row"><span>?ÅÌíàÎ™?</span> <strong>{{ state.selectedItemDetail.product_name }}</strong></div>
                <div class="data-row"><span>Í≤∞Ï†úÍ∏àÏï°:</span> <strong>{{ state.selectedItemDetail.total_amount?.toLocaleString() }}??/strong></div>
                <div class="data-row"><span>Î∞∞ÏÜ°?ÅÌÉú:</span> <span class="status-tag" :class="state.selectedItemDetail.delivery_status">{{ state.selectedItemDetail.delivery_status === 'preparing' ? 'Î∞∞ÏÜ°Ï§ÄÎπ? : state.selectedItemDetail.delivery_status === 'shipping' ? 'Î∞∞ÏÜ°Ï§? : 'Î∞∞ÏÜ°?ÑÎ£å' }}</span></div>
              </div>
            </div>

            <div class="detail-section">
              <h4 class="detail-sec-title">?ë§ Ï£ºÎ¨∏???ïÎ≥¥</h4>
              <div class="detail-data-box">
                <div class="data-row"><span>?¥Î©î??</span> {{ state.selectedItemDetail.user_email }}</div>
                <div class="data-row" v-if="state.selectedItemDetail.buyer_name"><span>?¥Î¶Ñ:</span> {{ state.selectedItemDetail.buyer_name }}</div>
                <div class="data-row" v-if="state.selectedItemDetail.buyer_phone"><span>?∞ÎùΩÏ≤?</span> {{ state.selectedItemDetail.buyer_phone }}</div>
              </div>
            </div>

            <div class="detail-section">
              <h4 class="detail-sec-title">?öö Î∞∞ÏÜ°ÏßÄ ?ïÎ≥¥</h4>
              <div class="detail-data-box">
                <div class="data-row"><span>?òÎ†π??</span> {{ state.selectedItemDetail.receiver_name }}</div>
                <div class="data-row"><span>?∞ÎùΩÏ≤?</span> {{ state.selectedItemDetail.receiver_phone }}</div>
                <div class="data-row"><span>Ï£ºÏÜå:</span> {{ state.selectedItemDetail.shipping_address }}</div>
              </div>
            </div>
          </div>
          ...
          <template v-else>
            ...
            <div v-if="isAdmin && state.selectedItemDetail?.category" class="admin-reply-section">
              <textarea v-model="state.adminReplyText" placeholder="?µÎ????ÖÎ†•?òÏÑ∏??.."></textarea>
              <button @click="submitAdminReply(state.selectedItemDetail.id)" class="primary-btn">?µÎ? ?Ä??/button>
            </div>
...
<style>
/* ?åÌÅ¨ Í∏∞Î∞ò ?ÑÎ¶¨ÎØ∏ÏóÑ ?§ÌÅ¨ ?åÎßà Î≥Ä??*/
:root {
  --tech-bg: #02040a; /* ?îÏ≤≠?òÏã† ?¥Î?ÏßÄ ?§Ïóê ÎßûÏ∂ò ÍπäÏ? ?§ÌÅ¨ ?§Ïù¥Îπ?Î∏îÎûô */
  --tech-nav-bg: #02040a;
  --tech-text: #FFFFFF;
  --tech-muted: #9DA1B4;
...
.my-page-icon-wrapper .my-text-label {
  position: absolute;
  bottom: 4px; /* Ï°∞Í∏à ???ÑÎ°ú ?¨Î†§???ÑÏù¥ÏΩòÍ≥º ??ÎßéÏù¥ Í≤πÏπòÍ≤?Ï°∞Ï†ï */
  right: -2px; 
  font-size: 10px;
  font-weight: 900;
  color: #fff;
